...
class RPSGame {
	...
	parseWin(winner, loser, inactivity) {
		...
		if (this.gameType === "bucks") {
			...
			winner.send(`|pm|~Rock/Paper/Scissors${this.gameType !== "ladderRPSLS" ? `` : `/Lizard/Spock`} Host|${winner.userid}|/html ${WL.nameColor(winner.name, true)}: ${winnerPoints.toLocaleString()} --> ${winnerFinalPoints.toLocaleString()}<br />${WL.nameColor(loser.name, true)}: ${loserPoints.toLocaleString()} --> ${loserFinalPoints.toLocaleString()}`);
			loser.send(`|pm|~Rock/Paper/Scissors${this.gameType !== "ladderRPSLS" ? `` : `/Lizard/Spock`} Host|${loser.userid}|/html ${WL.nameColor(winner.name, true)}: ${winnerPoints.toLocaleString()} --> ${winnerFinalPoints.toLocaleString()}<br />${WL.nameColor(loser.name, true)}: ${loserPoints.toLocaleString()} --> ${loserFinalPoints.toLocaleString()}`);
		}
	}
}
