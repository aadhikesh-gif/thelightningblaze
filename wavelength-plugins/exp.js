		...
		EXP.readExp(user.userid, totalExp => {
			...
			EXP.writeExp(user.userid, amount, newTotal => {
				...
				if (oldLevel < level) {
					...
					switch (level) {
					case 25:
						Economy.logTransaction(`${user.name} received a p profile music for reaching level ${level}.`);
						Monitor.log(`${user.userid} has earned a profile music for reaching level ${level}!`);
						if (!user.tokens) user.tokens = {};
						user.tokens.music = true;
						reward = `a and Profile Music. To claim your profile music, /usetoken music, [song url], [title of the song] respectively.`;
						break;
					case 30:
		...
		if (target || !target && this.broadcasting) {
			...
			EXP.readExp(user.userid, exp => {
				this.sendReplyBox(
					...
					"Level 5 unlocks a free Custom Avatar <br /><br />" +
					"Level 10 unlocks a free Profile Title <br /><br />" +
					"Level 15 unlocks a free Custom Color. <br /><br />" +
					"Level 20 unlocks a free Custom Userlist Icon. <br /><br />" +
					"Level 25 unlocks a free Profile Music. <br /><br />" +
					"Level 30 unlocks a free Profile Background  <br /><br />" +
